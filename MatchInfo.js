...
const MatchInfo = () => {
    ...
    const seasonFromQuery = new URLSearchParams(window.location.search).get("season");
    const season = seasonFromQuery || localStorage.getItem("iplSeason") || "2026";
    ...
    useEffect(() => {
        const intervalId = setInterval(() => {
            async function fetchMatchData() {
                try {
                    const response = await fetch(`${API_BASE}/api/iplscore/${matchId}?season=${encodeURIComponent(season)}`);
