...
const IplSchedule = ({ schedule, season }) => {
    ...
    if (schedule) {
        ...
        const dates = schedule
            .map((m) => {
                if (m.MATCH_COMMENCE_START_DATE) return m.MATCH_COMMENCE_START_DATE.split(" ")[0];
                if (m.MatchDateNew) return new Date(m.MatchDateNew).toISOString().split("T")[0];
                return null;
            })
            .filter(Boolean)
            .sort();
    ...
    return (
        <div className="ipl-schedule-container scrollable-container">
            ...
            {filterDate && (
                <button className="clear-date-btn" onClick={() => setFilterDate("")}>Clear Date</button>
            )}
                ...
                {filteredSchedule.map((match, index) => {
                    return (
                        <div key={index} className="match-card">
                            ...
                            <button
                                onClick={() => navigate(`/user/match-info/${match.MatchID}?season=${encodeURIComponent(season || "2025")}`)}
                            >
                                View Match Info
                            </button>
                        </div>
